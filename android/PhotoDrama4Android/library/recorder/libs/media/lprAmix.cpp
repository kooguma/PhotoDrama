...
static int init_filter_graph(AVFilterGraph **graph, AVFilterContext **src0, AVFilterContext **src1,
    ...
    av_opt_set(abuffersink_ctx, "channel_layout", (const char *) ch_layout, AV_OPT_SEARCH_CHILDREN);
...
static int open_output_file(const char *filename,
    ...
    /** initialize the stream parameters with demuxer information */
    error = avcodec_parameters_to_context(avctx, (*output_format_context)->streams[0]->codecpar);
        ...
        if (data_present_in_graph) {
            ...
            while (1) {
                ...
                av_log(NULL, AV_LOG_INFO,
                       "remove %d samples from sink (%d Hz, time=%f, ttime=%f)\n",
...
static int mix(const char *input1, const char *input2, const char *output) {
    av_log_set_level(AV_LOG_VERBOSE);


    int err;

    if (open_input_file(input1, &input_format_context_0, &input_codec_context_0) < 0) {
        av_log(NULL, AV_LOG_ERROR, "Error while opening file 1\n");
        exit(1);
    }
    av_dump_format(input_format_context_0, 0, input1, 0);

    if (open_input_file(input2, &input_format_context_1, &input_codec_context_1) < 0) {
        av_log(NULL, AV_LOG_ERROR, "Error while opening file 2\n");
        exit(1);
    }
    av_dump_format(input_format_context_1, 0, input2, 0);
    ...
    err = open_output_file(output, input_codec_context_0, &output_format_context,
...
Java_com_loopeer_media_VideoMixer_nativeMix(JNIEnv *env, jobject instance, jstring input1_,
    ...
    const char *input1 = env->GetStringUTFChars(input1_, 0);
    const char *input2 = env->GetStringUTFChars(input2_, 0);
    const char *output = env->GetStringUTFChars(output_, 0);

    mix(input1, input2, output);

    env->ReleaseStringUTFChars(input1_, input1);
    env->ReleaseStringUTFChars(input2_, input2);
