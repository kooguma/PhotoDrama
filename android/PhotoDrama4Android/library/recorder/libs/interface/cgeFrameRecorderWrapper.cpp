...
JNIEXPORT void JNICALL Java_org_wysaid_nativePort_FrameRecorder_nativeAddAudioFrame
		(JNIEnv *env, jobject, jlong addr, jstring filename)
{
	const char* filenameStr = env->GetStringUTFChars(filename, 0);
	CGEFrameRecorder* recorder = (CGEFrameRecorder*)addr;
	recorder->addAudioFrame(filenameStr);
