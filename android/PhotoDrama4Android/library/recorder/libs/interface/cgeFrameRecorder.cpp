...
//				assert(bufferSize != 0); // bufferSize must be set!
	...
	void CGEFrameRecorder::addAudioFrame(const char* filename)
	{
//		m_encoder->srcM = filename;
		m_encoder->decodeAudioFile(filename);
//		m_encoder-> audioDecode(filename);
	}
	...
	bool CGEFrameRecorder::startRecording(int fps, const char* filename, int bitRate)
	{
		...
		if(!m_encoder->init(filename, fps, m_dstSize.width, m_dstSize.height, true, bitRate))
		{
			delete m_encoder;
			m_encoder = nullptr;
			CGE_LOG_ERROR("CGEFrameRecorder::startRecording - start recording failed! %d : %d : %d : %d : %s", m_dstSize.width, m_dstSize.height, fps, bitRate, filename);
			return false;
		}
	...
	{
		CGE_LOG_INFO("decodeAudioFile %s", m_encoder->srcM.c_str());

//		m_encoder->decodeAudioFile(m_encoder->srcM.c_str());
