...
public final class CameraPermissionUtils {
    ...
    public static void checkCameraPermission(Activity activity) {
        if (!hasSelfPermissions(activity, CAMERA_PERMISSIONS[0])) {
    ...
    public static boolean hasCameraPermission(Context context) {
        int perm = context.checkCallingOrSelfPermission(CAMERA_PERMISSIONS[0]);
        return perm == PackageManager.PERMISSION_GRANTED;
    }

    public static void requestCameraPermissions(Fragment fragment) {
        fragment.requestPermissions(CAMERA_PERMISSIONS, REQUEST_CAMERA_PERMISSION);
    }
}
