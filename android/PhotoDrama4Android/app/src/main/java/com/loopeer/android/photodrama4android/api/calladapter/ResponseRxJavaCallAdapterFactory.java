 ...
 * for 2XX responses and calls {@code onError} with {@link retrofit2.adapter.rxjava.HttpException}
 ...
 * </ul>
 */
public final class ResponseRxJavaCallAdapterFactory extends CallAdapter.Factory {
    private static final String TAG = "RxJavaCallAdapter";
     ...
     * by default.
     */
    public static ResponseRxJavaCallAdapterFactory create(Context context) {
        return new ResponseRxJavaCallAdapterFactory(context, null);
    }
     ...
     */
    public static ResponseRxJavaCallAdapterFactory createWithScheduler(Context context, Scheduler scheduler) {
        if (scheduler == null) throw new NullPointerException("scheduler == null");
        return new ResponseRxJavaCallAdapterFactory(context, scheduler);
    }
    ...
    private ResponseRxJavaCallAdapterFactory(Context context, Scheduler scheduler) {
    ...
    private CallAdapter<Observable<?>> getCallAdapter(Type returnType, Scheduler scheduler) {
        ...
        if (rawObservableType == BaseResponse.class) {
            return new AppResponseCallAdapter(observableType, scheduler);
        }
    ...
    final class AppResponseCallAdapter implements CallAdapter<Observable<?>> {

        private final Type responseType;
        private final Scheduler scheduler;


        AppResponseCallAdapter(Type responseType, Scheduler scheduler) {
            this.responseType = responseType;
            this.scheduler = scheduler;
        }
        ...
        public <R> Observable<R> adapt(Call<R> call) {
            Observable<R> observable = Observable.create(new CallOnSubscribe<>(call)) //
                    .lift(OperatorMapBaseResponseToBodyOrError.<R>instance())
        ...
        private void checkNetError(Throwable throwable) {
            if (BuildConfig.DEBUG) {
                ...
                Observable.just("error:")
                    .observeOn(AndroidSchedulers.mainThread())
                    .subscribe(s -> Log.e(TAG, throwable.getMessage() == null ? "throwable null" : throwable.getMessage())
                );
            }
            Observable.just(throwable)
                    .filter(throwable1 -> throwable1 instanceof IOException)
                    .observeOn(AndroidSchedulers.mainThread()).subscribe(
                    e -> showToast("当前网络不可用，请检查网络设置")
            );
        }
